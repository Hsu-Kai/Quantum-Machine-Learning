...
def conv_net_energy(weights, last_layer_weights, features):
    # convolution net for predicting forces
    ...
    return qml.expval((eV_to_hatree)*(qml.PauliX(0)@qml.PauliX(4)+qml.PauliY(0)@qml.PauliY(4)+qml.PauliZ(0)@qml.PauliZ(4)))
...
@jax.jit
def compute_energy_out(weights, weights_last, features):
    """Computes the output of qcnn"""
    energy_out = lambda weights, weights_last, features: conv_net_energy(weights, weights_last, features)
    return jax.vmap(energy_out, in_axes=(None, None, 0), out_axes=0)(
        weights, weights_last, features
    )

def compute_energy_accuracy(weights, weights_last, features, E_train):
    """Computes the accuracy over the provided features"""
    E_pred = compute_energy_out(weights, weights_last, features)
    energy_loss = jnp.average((E_pred - E_train)**2) 
    return 1.0 - jnp.sqrt(energy_loss) / jax.numpy.linalg.norm(E_train)


def compute_energy_cost(weights, weights_last, features, E_train):
...
def train_qcnn(n_train, n_test, n_epochs):
    ...
    for step in range(n_epochs):
        ...
        train_cost, grad_circuit = energy_value_and_grad(weights, weights_last, x_train, y_train)
        updates, opt_state = optimizer.update(grad_circuit, opt_state)
        weights, weights_last = optax.apply_updates((weights, weights_last), updates)
        train_energy_out = compute_energy_out(weights, weights_last, x_train)
        ...
        train_acc = compute_energy_accuracy(weights, weights_last, x_train, y_train)     
        train_acc_epochs.append(train_acc)

        # compute accuracy and cost on testing data
        test_energy_out = compute_energy_out(weights, weights_last, x_test)
        ...
        test_cost = jnp.average((test_energy_out - y_test)**2)
...
n_epochs = 1000
n_reps = 10
final_weights, final_weights_last = [], []
...
results_df.to_pickle("/work/hsukaicheng/NiCoTiZrHf/train_energies/QCNN_v24/v24_03/qcnn_results_df_v24")
np.save("/work/hsukaicheng/NiCoTiZrHf/train_energies/QCNN_v24/v24_03/qcnn_final_weights_v24", final_weights)
np.save("/work/hsukaicheng/NiCoTiZrHf/train_energies/QCNN_v24/v24_03/qcnn_final_weights_last_v24", final_weights_last)
