...
D_max = max(list([D_all[i].max() for i in range(len(D_all))]))
bin_edges = np.linspace(np.nextafter(D_min, -np.inf), D_max, n_bin+1)
bin_mean = np.array([0.5*(bin_edges[i] + bin_edges[i+1]) for i in range(len(bin_edges)-1)])
D_all_binning = descriptors.binned_descriptors(n_bin, D_all, binned_bispectrum=False)
D_all_binning = D_all_binning.astype('float')
for i in range(len(D_all_binning)):
	for j in range(len(D_all_binning[i])):
		D_all_binning[i][j] = D_all_binning[i][j].astype('float')*bin_mean
D_all_binning = D_all_binning.reshape(len(Training_Sets_All[0]), -1)
...
D_train, D_test = D_all_binning[train_indices], D_all_binning[test_indices]
